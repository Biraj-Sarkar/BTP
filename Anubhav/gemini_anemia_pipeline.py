"""Conjunctiva preprocessing pipeline for anemia detection.

This script is designed for the current dataset layout in:
    /Users/yatikajena/Desktop/AnemiaDetection/dataset anemia/India

What it does:
1. Loads only the raw eye JPG images from the Indian subset.
2. Segments a conjunctiva candidate region without using the provided masks.
3. Crops the region and pads it to a perfect square to preserve the aspect ratio.
4. Computes simple quality checks for blur and exposure variation.
5. Saves masks, crops, normalized images, overlays, and a manifest CSV.
6. Includes regression imbalance utilities for the later Hb training stage.

Paper-backed design choice:
* For the long-term segmentation model, the strongest conference-backed choice
  for this project is a promptable foundation model such as SAM (CVPR 2023)
  for mask proposal, or Mask2Former (CVPR 2022) if you move to supervised fine-
  tuning. For this demo script, a deterministic classical fallback is used so
  the pipeline can run immediately on the local sample images without needing a
  large external checkpoint download.

The code is intentionally modular so the segmentation backend can be swapped
later without changing the rest of the preprocessing pipeline.
"""
...
except Exception:  # pragma: no cover - optional for demo use
    torch = None
    nn = object
    F = object
    Dataset = object
...
def gray_world_white_balance(image_rgb: np.ndarray) -> np.ndarray:
    ...
    balanced = np.clip(image * scale.reshape(1, 1, 3), 0, 255)
...
def heuristic_conjunctiva_mask(image_rgb: np.ndarray) -> np.ndarray:
    """Color-aware conjunctiva candidate mask.

    The conjunctiva/sclera region is typically bright and relatively neutral in
    CIELAB space. This heuristic keeps bright, low-chroma pixels and then keeps
...
def grabcut_mask(image_rgb: np.ndarray) -> np.ndarray:
    """Fallback mask proposal with GrabCut.

    This is not the final research target, but it is a deterministic, runnable
    baseline when a heavy pretrained segmentation checkpoint is unavailable.
    """
...
def crop_to_mask(image_rgb: np.ndarray, mask: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    left, top, right, bottom = tight_bbox(mask)
    cropped_img = image_rgb[top:bottom, left:right]
...
def process_image(image_path: Path, config: PipelineConfig, sample_name: str) -> dict:
    raw_rgb = read_rgb(image_path)
    raw_rgb = resize_image(raw_rgb, config.target_size)

    balanced_rgb = gray_world_white_balance(raw_rgb)
    mask, backend = segment_conjunctiva(balanced_rgb)
...
def preprocess_hb_image(image_path: Path, config: PipelineConfig, segmenter: Any) -> dict:
    ...
    balanced_rgb = gray_world_white_balance(resized_rgb)
    mask, _ = segment_conjunctiva(balanced_rgb, segmenter=segmenter)
    masked_rgb = apply_mask(balanced_rgb, mask)
    cropped_rgb, cropped_mask = crop_to_mask(masked_rgb, mask)
    cropped_mask = (cropped_mask > 0).astype(np.uint8) * 255
    source_rgb = cropped_rgb if cropped_rgb.size else masked_rgb
    normalized_rgb = source_rgb
    return {
        "raw_rgb": raw_rgb,
...
def augment_hb_image(image_rgb: np.ndarray) -> np.ndarray:
...
def save_prediction_package(
    ...
    segmented_dir = package_root / "segmented_regions"
    ...
    for record, prediction in zip(records, predictions):
        ...
        write_rgb(prep["cropped_rgb"] if prep["cropped_rgb"].size else prep["masked_rgb"], segmented_dir / f"{sample_name}_segment.png")
        write_rgb(prep["normalized_rgb"], normalized_dir / f"{sample_name}_normalized.png")
...
def regression_sample_weights(targets: Sequence[float], bins: int = 10) -> np.ndarray:
    """Inverse-frequency sample weights for Hb regression.

    This is a lightweight replacement for label-distribution smoothing when you
    do not yet have a full training set. It is appropriate for imbalanced
    regression because rare Hb ranges get higher weights.
    """
...
class ConjunctivaDemoDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        mask, _ = segment_conjunctiva(gray_world_white_balance(image))
...
class HbRegressionDataset(Dataset):
    ...
    def _preprocess(self, image_path: Path) -> np.ndarray:
        ...
        return cv2.resize(prep["normalized_rgb"], self.config.regression_image_size, interpolation=cv2.INTER_AREA)

    def __getitem__(self, index: int):
        record = self.records[index]
        image_rgb = self._preprocess(record["image_path"])
        if self.augment:
            image_rgb = augment_hb_image(image_rgb)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("/Users/yatikajena/Desktop/AnemiaDetection/conjunctiva_pipeline_outputs_v6"),
